...
const FILL_EXERCISES = [
  ...
  {id:'f9', sentence:'He ____ _____ (run) — his leg hurts.', answer:"can't / run", hint:'Negative + base verb', explanation:'CAN\'T + base verb. No -ing, no -s! / ¡Sin -ing, sin -s!'},
  {id:'f10', sentence:'You ____ use my pencil.', answer:'can', hint:'Permission / Permiso dado', explanation:'CAN is used to give permission. / CAN se usa para dar permiso.'},
  {id:'f11', sentence:'Tom ____ _____ (play) the guitar.', answer:'can / play', hint:'Ability + base verb', explanation:'CAN + base verb. She can PLAY (not plays or playing).'},
...
function initSort() {
  ...
  bank.innerHTML = shuffled.map(item => `
    <div class="sort-card" id="sort-card-${item.idx}" onclick="sortCardClick(${item.idx},'${item.type}')">
      ${item.text}
    </div>
  `).join('');
}

function sortCardClick(idx, type) {
  if (STATE.sortSelected === null) {
    STATE.sortSelected = {idx, type};
...
function checkSort() {
  // Evaluate based on where cards were placed
  const fb = document.getElementById('sortFeedback');
  fb.className = 'feedback show correct';
  fb.innerHTML = '💡 Click each card to choose where it belongs! Move cards to CAN or CAN\'T column by clicking the column buttons below each card.';
  // Simple: show answers
  const bank = document.getElementById('sortBank');
  bank.innerHTML = SORT_ITEMS.map((item, i) => `
    <div class="sort-card" style="border-color:${item.type==='can'?'var(--green)':'var(--red)'};">
      ${item.text} <span style="font-size:0.8rem;">(${item.type})</span>
    </div>
  `).join('');
  ...
  container.innerHTML = POLICE_SCENARIOS.map((s, i) => `
    <div class="scenario-card" id="police-card-${i}">
      ...
      <div class="perm-choices">
        ...
        <button class="perm-btn cant-btn" id="perm-cant-${i}" onclick="answerPolice(${i},\"can't\")">❌ CAN'T</button>
      </div>
  ...
  container.innerHTML = SCHOOL_RULES.map((rule, i) => `
    <div class="rule-item" id="rule-item-${i}">
      ...
      <div class="rule-choices">
        ...
        <button class="rule-btn cant-r" id="rule-cant-${i}" onclick="answerRule(${i},\"can't\")">CAN'T ❌</button>
      </div>
    </div>
  `).join('');
}
...
function answerRule(idx, answer) {
  ...
  const isCorrect = answer === rule.answer;
